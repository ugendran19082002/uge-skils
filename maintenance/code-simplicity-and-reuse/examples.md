# Code Simplicity & Reuse — Worked Examples

Realistic, end-to-end walkthroughs of the `code-simplicity-and-reuse` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Code Simplicity & Reuse on a greenfield feature

**Context.** A team is starting new work and needs code simplicity & reuse done properly the first time.
...
## Example 2: Applying Code Simplicity & Reuse to an existing system

**Context.** An established system needs code simplicity & reuse retrofitted to reduce risk.
