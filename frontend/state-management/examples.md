# State Management — Worked Examples

Realistic, end-to-end walkthroughs of the `state-management` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying State Management on a greenfield feature

**Context.** A team is starting new work and needs state management done properly the first time.
...
## Example 2: Applying State Management to an existing system

**Context.** An established system needs state management retrofitted to reduce risk.
