# Data Pipeline Design — Worked Examples

Realistic, end-to-end walkthroughs of the `data-pipeline-design` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Data Pipeline Design on a greenfield feature

**Context.** A team is starting new work and needs data pipeline design done properly the first time.
...
## Example 2: Applying Data Pipeline Design to an existing system

**Context.** An established system needs data pipeline design retrofitted to reduce risk.
