# Model Fine-Tuning — Worked Examples

Realistic, end-to-end walkthroughs of the `model-finetuning` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Model Fine-Tuning on a greenfield feature

**Context.** A team is starting new work and needs model fine-tuning done properly the first time.
...
## Example 2: Applying Model Fine-Tuning to an existing system

**Context.** An established system needs model fine-tuning retrofitted to reduce risk.
