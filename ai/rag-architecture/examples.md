# RAG Architecture — Worked Examples

Realistic, end-to-end walkthroughs of the `rag-architecture` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying RAG Architecture on a greenfield feature

**Context.** A team is starting new work and needs rag architecture done properly the first time.
...
## Example 2: Applying RAG Architecture to an existing system

**Context.** An established system needs rag architecture retrofitted to reduce risk.
