...
Realistic, end-to-end walkthroughs of the `model-selection` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Model Selection on a greenfield feature

**Context.** A team is starting new work and needs model selection done properly the first time.
...
## Example 2: Applying Model Selection to an existing system

**Context.** An established system needs model selection retrofitted to reduce risk.
