# Vector Database Design — Worked Examples

Realistic, end-to-end walkthroughs of the `vector-database-design` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Vector Database Design on a greenfield feature

**Context.** A team is starting new work and needs vector database design done properly the first time.
...
## Example 2: Applying Vector Database Design to an existing system

**Context.** An established system needs vector database design retrofitted to reduce risk.
