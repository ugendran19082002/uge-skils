...
Realistic, end-to-end walkthroughs of the `threat-modeling` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Threat Modeling on a greenfield feature

**Context.** A team is starting new work and needs threat modeling done properly the first time.
...
## Example 2: Applying Threat Modeling to an existing system

**Context.** An established system needs threat modeling retrofitted to reduce risk.
