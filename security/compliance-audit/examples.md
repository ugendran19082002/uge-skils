# Compliance Audit — Worked Examples

Realistic, end-to-end walkthroughs of the `compliance-audit` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Compliance Audit on a greenfield feature

**Context.** A team is starting new work and needs compliance audit done properly the first time.
...
## Example 2: Applying Compliance Audit to an existing system

**Context.** An established system needs compliance audit retrofitted to reduce risk.
