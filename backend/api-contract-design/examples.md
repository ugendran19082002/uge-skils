# API Contract Design — Worked Examples

Realistic, end-to-end walkthroughs of the `api-contract-design` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying API Contract Design on a greenfield feature

**Context.** A team is starting new work and needs api contract design done properly the first time.
...
## Example 2: Applying API Contract Design to an existing system

**Context.** An established system needs api contract design retrofitted to reduce risk.
