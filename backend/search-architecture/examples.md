# Search Architecture — Worked Examples

Realistic, end-to-end walkthroughs of the `search-architecture` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Search Architecture on a greenfield feature

**Context.** A team is starting new work and needs search architecture done properly the first time.
...
## Example 2: Applying Search Architecture to an existing system

**Context.** An established system needs search architecture retrofitted to reduce risk.
