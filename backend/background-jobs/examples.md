# Background Jobs — Worked Examples

Realistic, end-to-end walkthroughs of the `background-jobs` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Background Jobs on a greenfield feature

**Context.** A team is starting new work and needs background jobs done properly the first time.
...
## Example 2: Applying Background Jobs to an existing system

**Context.** An established system needs background jobs retrofitted to reduce risk.
