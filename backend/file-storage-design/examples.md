# File Storage Design — Worked Examples

Realistic, end-to-end walkthroughs of the `file-storage-design` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying File Storage Design on a greenfield feature

**Context.** A team is starting new work and needs file storage design done properly the first time.
...
## Example 2: Applying File Storage Design to an existing system

**Context.** An established system needs file storage design retrofitted to reduce risk.
