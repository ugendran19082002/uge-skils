# Notification System — Worked Examples

Realistic, end-to-end walkthroughs of the `notification-system` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Notification System on a greenfield feature

**Context.** A team is starting new work and needs notification system done properly the first time.
...
## Example 2: Applying Notification System to an existing system

**Context.** An established system needs notification system retrofitted to reduce risk.
