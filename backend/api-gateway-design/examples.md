# API Gateway Design — Worked Examples

Realistic, end-to-end walkthroughs of the `api-gateway-design` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying API Gateway Design on a greenfield feature

**Context.** A team is starting new work and needs api gateway design done properly the first time.
...
## Example 2: Applying API Gateway Design to an existing system

**Context.** An established system needs api gateway design retrofitted to reduce risk.
