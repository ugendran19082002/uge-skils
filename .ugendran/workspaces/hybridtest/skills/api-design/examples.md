...
Realistic, end-to-end walkthroughs of the `api-design` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying API Design on a greenfield feature

**Context.** A team is starting new work and needs api design done properly the first time.
...
## Example 2: Applying API Design to an existing system

**Context.** An established system needs api design retrofitted to reduce risk.
