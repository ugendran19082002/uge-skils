...
Realistic, end-to-end walkthroughs of the `security-architecture` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Security Architecture on a greenfield feature

**Context.** A team is starting new work and needs security architecture done properly the first time.
...
## Example 2: Applying Security Architecture to an existing system

**Context.** An established system needs security architecture retrofitted to reduce risk.
