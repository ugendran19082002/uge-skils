...
Realistic, end-to-end walkthroughs of the `cost-estimation` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Cost Estimation on a greenfield feature

**Context.** A team is starting new work and needs cost estimation done properly the first time.
...
## Example 2: Applying Cost Estimation to an existing system

**Context.** An established system needs cost estimation retrofitted to reduce risk.
