# Event-Driven Architecture — Worked Examples

Realistic, end-to-end walkthroughs of the `event-driven-architecture` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Event-Driven Architecture on a greenfield feature

**Context.** A team is starting new work and needs event-driven architecture done properly the first time.
...
## Example 2: Applying Event-Driven Architecture to an existing system

**Context.** An established system needs event-driven architecture retrofitted to reduce risk.
