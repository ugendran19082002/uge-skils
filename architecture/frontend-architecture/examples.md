...
Realistic, end-to-end walkthroughs of the `frontend-architecture` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Frontend Architecture on a greenfield feature

**Context.** A team is starting new work and needs frontend architecture done properly the first time.
...
## Example 2: Applying Frontend Architecture to an existing system

**Context.** An established system needs frontend architecture retrofitted to reduce risk.
