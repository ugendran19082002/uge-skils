# Realtime Architecture — Worked Examples

Realistic, end-to-end walkthroughs of the `realtime-architecture` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Realtime Architecture on a greenfield feature

**Context.** A team is starting new work and needs realtime architecture done properly the first time.
...
## Example 2: Applying Realtime Architecture to an existing system

**Context.** An established system needs realtime architecture retrofitted to reduce risk.
