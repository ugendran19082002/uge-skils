# Multi-Tenant Architecture — Worked Examples

Realistic, end-to-end walkthroughs of the `multi-tenant-architecture` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Multi-Tenant Architecture on a greenfield feature

**Context.** A team is starting new work and needs multi-tenant architecture done properly the first time.
...
## Example 2: Applying Multi-Tenant Architecture to an existing system

**Context.** An established system needs multi-tenant architecture retrofitted to reduce risk.
