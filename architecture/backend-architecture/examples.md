# Backend Architecture — Worked Examples

Realistic, end-to-end walkthroughs of the `backend-architecture` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Backend Architecture on a greenfield feature

**Context.** A team is starting new work and needs backend architecture done properly the first time.
...
## Example 2: Applying Backend Architecture to an existing system

**Context.** An established system needs backend architecture retrofitted to reduce risk.
