# Testing Strategy — Worked Examples

Realistic, end-to-end walkthroughs of the `testing-strategy` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Testing Strategy on a greenfield feature

**Context.** A team is starting new work and needs testing strategy done properly the first time.
...
## Example 2: Applying Testing Strategy to an existing system

**Context.** An established system needs testing strategy retrofitted to reduce risk.
