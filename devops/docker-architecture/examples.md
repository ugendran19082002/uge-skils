# Docker Architecture — Worked Examples

Realistic, end-to-end walkthroughs of the `docker-architecture` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Docker Architecture on a greenfield feature

**Context.** A team is starting new work and needs docker architecture done properly the first time.
...
## Example 2: Applying Docker Architecture to an existing system

**Context.** An established system needs docker architecture retrofitted to reduce risk.
