# Server Hardening — Worked Examples

Realistic, end-to-end walkthroughs of the `server-hardening` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Server Hardening on a greenfield feature

**Context.** A team is starting new work and needs server hardening done properly the first time.
...
## Example 2: Applying Server Hardening to an existing system

**Context.** An established system needs server hardening retrofitted to reduce risk.
