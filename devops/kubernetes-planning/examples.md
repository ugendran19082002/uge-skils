# Kubernetes Planning — Worked Examples

Realistic, end-to-end walkthroughs of the `kubernetes-planning` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Kubernetes Planning on a greenfield feature

**Context.** A team is starting new work and needs kubernetes planning done properly the first time.
...
## Example 2: Applying Kubernetes Planning to an existing system

**Context.** An established system needs kubernetes planning retrofitted to reduce risk.
