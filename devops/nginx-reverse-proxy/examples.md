# Nginx Reverse Proxy — Worked Examples

Realistic, end-to-end walkthroughs of the `nginx-reverse-proxy` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Nginx Reverse Proxy on a greenfield feature

**Context.** A team is starting new work and needs nginx reverse proxy done properly the first time.
...
## Example 2: Applying Nginx Reverse Proxy to an existing system

**Context.** An established system needs nginx reverse proxy retrofitted to reduce risk.
