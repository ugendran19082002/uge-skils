# Secrets Management — Worked Examples

Realistic, end-to-end walkthroughs of the `secrets-management` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Secrets Management on a greenfield feature

**Context.** A team is starting new work and needs secrets management done properly the first time.
...
## Example 2: Applying Secrets Management to an existing system

**Context.** An established system needs secrets management retrofitted to reduce risk.
