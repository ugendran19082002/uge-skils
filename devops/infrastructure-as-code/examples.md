...
Realistic, end-to-end walkthroughs of the `infrastructure-as-code` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Infrastructure as Code on a greenfield feature

**Context.** A team is starting new work and needs infrastructure as code done properly the first time.
...
## Example 2: Applying Infrastructure as Code to an existing system

**Context.** An established system needs infrastructure as code retrofitted to reduce risk.
