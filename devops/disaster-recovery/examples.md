# Disaster Recovery — Worked Examples

Realistic, end-to-end walkthroughs of the `disaster-recovery` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Disaster Recovery on a greenfield feature

**Context.** A team is starting new work and needs disaster recovery done properly the first time.
...
## Example 2: Applying Disaster Recovery to an existing system

**Context.** An established system needs disaster recovery retrofitted to reduce risk.
