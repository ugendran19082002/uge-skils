# Backup & Recovery — Worked Examples

Realistic, end-to-end walkthroughs of the `backup-recovery` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Backup & Recovery on a greenfield feature

**Context.** A team is starting new work and needs backup & recovery done properly the first time.
...
## Example 2: Applying Backup & Recovery to an existing system

**Context.** An established system needs backup & recovery retrofitted to reduce risk.
