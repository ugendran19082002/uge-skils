# Caching Strategy — Worked Examples

Realistic, end-to-end walkthroughs of the `caching-strategy` skill. Each example
follows the OpenSpec phases (Discover → Specify → Design → Validate → Deliver).

## Example 1: Applying Caching Strategy on a greenfield feature

**Context.** A team is starting new work and needs caching strategy done properly the first time.
...
## Example 2: Applying Caching Strategy to an existing system

**Context.** An established system needs caching strategy retrofitted to reduce risk.
